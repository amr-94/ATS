                        ...
                        <form method="POST" action="{{ route('candidate.profile.update') }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    id="name" name="name" value="{{ old('name', $candidate->name) }}">
                                @error('name')
                            ...
                            <div class="mb-3">
                                <label for="resume" class="form-label">Upload CV (PDF/DOC/DOCX)</label>
                                <input type="file" class="form-control @error('resume') is-invalid @enderror"
                                    id="resume" name="resume">
                                @if ($candidate->resume_path)
                                    <p class="mt-2">Current Resume:
                                        <a href="{{ asset('storage/' . $candidate->resume_path) }}" target="_blank">View</a>
                                    </p>
                                @endif
                                @error('resume')
                                    <div class="invalid-feedback">{{ $message }}</div>
